...
def get_no_target_model_and_assets():
    """Returns a tuple containing the model XML string and a dict of assets."""
    return common.read_model('reacher_no_target.xml'), common.ASSETS
...
@SUITE.add('benchmarking', 'no_target')
def no_target(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
    """Returns reacher with sparse reward with 5e-2 tol and no target."""
    physics = Physics.from_xml_string(*get_no_target_model_and_assets())
    task = ReacherNoTarget(random=random)
    environment_kwargs = environment_kwargs or {}
    return control.Environment(
        physics, task, time_limit=time_limit, **environment_kwargs)
...
class ReacherNoTarget(base.Task):

    def __init__(self, random=None):

        super(ReacherNoTarget, self).__init__(random=random)
    ...
    def initialize_episode(self, physics):
        ...
        super(ReacherNoTarget, self).initialize_episode(physics)
